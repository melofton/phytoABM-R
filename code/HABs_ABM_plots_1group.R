...
ts_keep <- seq(from = 0, to = 9770, by = 60)

plot_yloc <- read_csv("./model_output/ABM_depthByTimestep_163hr.csv") %>%
...
dates <- seq(from=as.POSIXct("2021-08-09 05:01"),to=as.POSIXct("2021-08-09 05:01")+9770*60,by="min", tz = "UTC") 
plot_dates <- tibble(dates[ts_keep])

plot_ts <- read_csv("./model_output/ABM_surfaceTimeseries_163hr.csv") %>%
  filter(timestep %in% ts_keep) %>%
