...
ts_keep <- seq(from = 0, to = 2810, by = 10)

plot_yloc <- read_csv("./model_output/ABM_depthByTimestep_47hr.csv") %>%
...
ggsave(DepthTime, filename = "./plot_output/depth_by_time_1pft.tif",height = 5, width = 3.33,
       units = "in", dpi = 300, dev = "tiff")

dates <- seq(from=as.POSIXct("2021-08-09 05:01"),to=as.POSIXct("2021-08-09 05:01")+2810*60,by="min", tz = "UTC") 
plot_dates <- tibble(dates[ts_keep])

plot_ts <- read_csv("./model_output/ABM_surfaceTimeseries_47hr.csv") %>%
  filter(timestep %in% ts_keep) %>%
